...
const ContactLanding: React.FC = () => {
  ...
  return (
    <div className="animate-fade-in bg-[#050505] text-white min-h-screen pt-20">
      ...
      <div className="max-w-6xl mx-auto px-6 mb-24">
        <h2 className="text-2xl font-bold text-center mb-10 text-gray-200">Nuestros Canales Principales</h2>
        <div className="grid md:grid-cols-4 gap-6">
          {[
            ...
            <div key={i} className="bg-[#111] p-6 rounded-2xl border border-white/5 hover:border-white/10 transition-colors group">
              <div className="w-10 h-10 bg-white/5 rounded-lg flex items-center justify-center text-gray-300 mb-4 group-hover:bg-white/10 group-hover:text-white transition-colors">
                {React.cloneElement(item.icon as React.ReactElement<any>, { size: 20 })}
              </div>
              <h3 className="font-bold text-lg mb-2">{item.name}</h3>
              <p className="text-xs text-gray-500 mb-6 h-10">{item.desc}</p>
      ...
      <div className="max-w-5xl mx-auto px-6 pb-24">
        <div className="bg-[#111] border border-white/10 rounded-2xl shadow-2xl overflow-hidden flex flex-col md:flex-row min-h-[600px]">
          
          {/* Left Sidebar: Event Details */}
          <div className="w-full md:w-1/3 p-8 border-b md:border-b-0 md:border-r border-white/10 bg-[#0a0a0a]">
            {step === 'calendar' ? (
               <>
                <div className="mb-6">
                   ...
                   <p className="text-gray-400 text-sm font-medium">Cristian Muñoz</p>
                   <h2 className="text-2xl font-bold text-white mt-1">1 hour Meeting</h2>
                   <p className="text-gray-500 text-sm mt-2">Let's build the future</p>
                </div>
               ...
               <div className="h-full flex flex-col">
                  <button onClick={() => setStep('calendar')} className="mb-6 w-10 h-10 rounded-full border border-white/10 flex items-center justify-center hover:bg-white/5 text-gray-400">
                     <ChevronLeft size={20} />
                  </button>
                  <p className="text-gray-400 text-sm font-medium">Cristian Muñoz</p>
                  <h2 className="text-xl font-bold text-white mt-1">1 hour Meeting</h2>
          ...
          <div className="w-full md:w-2/3 p-8 bg-[#111] relative">
              ...
              <div className="flex flex-col md:flex-row h-full gap-8">
                 ...
                 <div className="flex-1">
                    <div className="flex justify-between items-center mb-6">
                       ...
                       <div className="flex gap-2">
                          <button className="p-2 hover:bg-white/5 rounded-full text-gray-400"><ChevronLeft size={16} /></button>
                          <button className="p-2 hover:bg-white/5 rounded-full text-gray-400"><ChevronRight size={16} /></button>
                       </div>
                    </div>
                 ...
                 {selectedDate && (
                    <div className="w-full md:w-48 animate-fade-in border-l border-white/5 pl-8 md:pl-0 md:border-l-0">
                       <h3 className="text-sm font-medium text-gray-400 mb-4 text-center md:text-left">Lun {selectedDate}</h3>
                       <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                          {timeSlots.map(time => (
                             <button
                                key={time}
                                onClick={() => handleTimeSelect(time)}
                                className="w-full py-2.5 px-4 rounded-lg border border-white/10 text-white text-sm font-medium hover:border-white hover:bg-white/5 transition-all text-center"
                             >
                                {time}
                             </button>
                          ))}
            ...
            {step === 'form' && (
               <form onSubmit={handleFormSubmit} className="max-w-md mx-auto animate-fade-in space-y-6">
                  <div>
                     ...
                     <input required type="text" className="w-full bg-[#0a0a0a] border border-white/10 rounded-lg px-4 py-2.5 text-white focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-all" />
                  </div>
                  <div>
                     <label className="block text-sm font-medium text-gray-400 mb-1">Email *</label>
                     <input required type="email" className="w-full bg-[#0a0a0a] border border-white/10 rounded-lg px-4 py-2.5 text-white focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-all" />
                  </div>
                  <div>
                     <label className="block text-sm font-medium text-gray-400 mb-1">Notas Adicionales</label>
                     <textarea rows={4} className="w-full bg-[#0a0a0a] border border-white/10 rounded-lg px-4 py-2.5 text-white focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-all resize-none"></textarea>
                  </div>
                  
                  <button type="button" className="text-sm text-gray-400 hover:text-white flex items-center gap-1">
                     <Plus size={16} /> Añadir invitados
                  </button>

                  <div className="pt-4 flex justify-end gap-3">
                     <button type="button" onClick={() => setStep('calendar')} className="px-5 py-2.5 text-sm font-medium text-gray-400 hover:text-white transition-colors">Atrás</button>
